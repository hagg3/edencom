...
// See Hud.h. NULL = stock: the in-game menu screen always draws.
bool (*eden_hud_draw_menu_screen_hook)(void)=NULL;
//@implementation Hud
...
void Hud::layoutForScreen(){
   ...
   // }else{
    ...
    // Crouch button: same icon as jump, drawn mirrored on the X axis, sitting just to its left.
    rcrouchrender.origin.x=rjumprender.origin.x-50;
    rcrouchrender.origin.y=rjumprender.origin.y;
    rcrouchrender.size.width=45;
    rcrouchrender.size.height=45;
    rcrouchhit.origin.x=rcrouchrender.origin.x-1;
   ...
   //if(mode==MODE_PICK_BLOCK){
	...
	if(delayedtimer==1){
		...
		if(delayedaction==6){
           ...
           // printg("trying to exit\n!!!");
            ...
            Resources::getResources->playSound(S_EXIT_WORLD);
	...
	for(int i=0;i<MAX_TOUCHES;i++){
        if(touches[i].inuse==usage_id2&&touches[i].down==M_RELEASE){
            BOOL handled;
            Resources::getResources->playSound(S_MENU_BUTTON_RELEASE);
            if(inmenu){
        ...
        if(touches[i].inuse==0&&touches[i].down==M_DOWN){
            if(inmenu){
                   ...
                   inbox3(touches[i].mx,touches[i].my,&rtExit))
                {
                    ...
                    touches[i].down=M_NONE;
                    Resources::getResources->playSound(S_MENU_BUTTON_PRESS);
                }
            }
	...
	if(CROUCH_ENABLED){
		for(int i=0;i<MAX_TOUCHES;i++){
			if((touches[i].inuse==0||touches[i].inuse==usage_id)&&touches[i].down==M_DOWN){
				if(inbox2(touches[i].mx,touches[i].my,&rcrouchhit)){
					m_crouch=TRUE;
					inmenu=FALSE;
					touches[i].inuse=usage_id;
				}
...
BOOL Hud::handlePickMenu(int x,int y){
	...
	if(inbox2(x,y,&rsave)||inbox2(x,y,&rtSave)){
		...
		World::getWorld->fm->saveWorld();
        Resources::getResources->playSound(S_WORLD_SAVED);
...
void Hud::render(){
    ...
    // Crouch button: same icon/texture as jump, mirrored top<->bottom (flipX=TRUE) so the
    // up-pointing chevron points down. Hidden entirely when the "Crouch mode" experiment is off
    // (Settings_web.mm's "crouch" row) — the button doesn't just no-op, it disappears.
    extern BOOL CROUCH_ENABLED;
    if(CROUCH_ENABLED){
        if(m_crouch){
            rcrouchrender.pressed=TRUE;
            res->getTex(ICO_JUMP_ACTIVE)->drawButton(rcrouchrender,TRUE);
        }else{
            rcrouchrender.pressed=FALSE;
            tjump->drawButton(rcrouchrender,TRUE);
        }
    }
    ...
    // The corner icon above always draws — it is how the menu gets OPENED. Only the panel itself is
    // suppressible (see eden_hud_draw_menu_screen_hook in Hud.h).
    if((inmenu||at1>0) && (!eden_hud_draw_menu_screen_hook || eden_hud_draw_menu_screen_hook())){
...
void Hud::asetHome(){
    ...
    World::getWorld->terrain->startDynamics();
    Resources::getResources->playSound(S_WARP_HOME_LOCATION_SET);
    sb->setStatus(@"World Saved",3);
