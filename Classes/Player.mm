...
bool FLY_UP=true;
bool FLY_DOWN=true;
// Web port crouch key (held, Source-engine style — set from src/seam/Input_web.mm). Halves
// boxheight in Player::preupdate below so the player fits through 1-block-tall gaps/vents.
bool CROUCH_HELD=false;
// Experiments tab toggle (Settings_web.mm's "crouch" row, group "Experiments") — off by default.
...
// boxheight's approach to its target each frame (camera reads boxheight directly, so this alone
// smooths the eye-height change too — see cam->py below).
#define CROUCH_ANIM_TIME 0.25f
...
// Can the player's box grow from its current boxheight up to testHeight (feet planted at
// pos.y-boxheight/2) without poking into solid terrain? Only scans the newly-added slice above the
// current top, since anything below that already fit. Used to rate-limit the stand-up animation in
// preupdate so releasing crouch under a 1-block ceiling freezes the growth instead of snapping the
// box into the ceiling block and popping the player through the world.
BOOL Player::canFit(float testHeight){
    float feet=pos.y-boxheight/2;
    float bot=feet+boxheight;
    float top=feet+testHeight;
    if(top<=bot)return TRUE;
    float left=pos.x-boxbase/2;
    float right=pos.x+boxbase/2;
    float front=pos.z-boxbase/2;
    float back=pos.z+boxbase/2;
    Polyhedra box=makeBox(left,right,back,front,bot,top);
    for(int x=(int)left;x<=(int)(right+.5f);x++){
        for(int z=(int)front;z<=(int)(back+.5f);z++){
            for(int y=(int)bot;y<=(int)(top+.5f);y++){
                ...
                if(type<=0)continue;
                if(blockinfo[type]&IS_LIQUID)continue;
                Polyhedra pbox2;
                if(type>=TYPE_STONE_RAMP1&&type<=TYPE_ICE_RAMP4){
                    pbox2=makeRamp(x,x+1,z+1,z,y,y+1,type%4);
                }else if(type>=TYPE_STONE_SIDE1&&type<=TYPE_ICE_SIDE4){
                    pbox2=makeSide(x,x+1,z+1,z,y,y+1,type%4);
                }else{
                    pbox2=makeBox(x,x+1,z+1,z,y,y+1);
                }
                if(collidePolyhedra(box,pbox2))return FALSE;
            }
        }
    }
    return TRUE;
...
BOOL Player::preupdate(float etime){
    // Crouch: ease boxheight toward its target (CROUCH_BOX_HEIGHT/STAND_BOX_HEIGHT) over
    // CROUCH_ANIM_TIME seconds, keeping the FEET (bot=pos.y-boxheight/2) planted by shifting pos.y
    // by half the per-frame height delta. Growing (standing up) is rate-limited by canFit() so that
    // releasing crouch under a 1-block ceiling just freezes the animation instead of snapping the
    // full-height box into the ceiling block — the old instant-toggle version popped the player
    // through the world here. Shrinking (crouching down) never collides with anything the player
    // wasn't already standing clear of, so it's never gated.
    BOOL wantCrouch=CROUCH_ENABLED&&(CROUCH_HELD||World::getWorld->hud->m_crouch);
    float targetHeight=wantCrouch?CROUCH_BOX_HEIGHT:STAND_BOX_HEIGHT;
    if(boxheight!=targetHeight){
        float maxDelta=(STAND_BOX_HEIGHT-CROUCH_BOX_HEIGHT)*etime/CROUCH_ANIM_TIME;
        float newHeight;
        if(targetHeight<boxheight){
            newHeight=boxheight-maxDelta;
            if(newHeight<targetHeight)newHeight=targetHeight;
        }else{
            newHeight=boxheight+maxDelta;
            if(newHeight>targetHeight)newHeight=targetHeight;
            if(!canFit(newHeight))newHeight=boxheight;
        }
        if(newHeight!=boxheight){
            pos.y-=(boxheight-newHeight)/2.0f;
            boxheight=newHeight;
        }
    }
    crouching=(boxheight!=STAND_BOX_HEIGHT);
    if(life<0){
        flash=1;
