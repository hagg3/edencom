...
TerrainChunk::TerrainChunk(const int* boundz,Terrain* terrain){
 ...
 //   memset(sblocks,0,sizeof(SmallBlock*)*CHUNK_SIZE3);
    ...
    rtn_vertices=0;
    rtn_vertices2=0;
    rtGeomHash=0;   // Phase N Stage 1 — same reason the two counts above are zeroed here: a chunk
    rtFullHash=0;   // is allocated out of a heap that has just held vertex data.
    rtvis_vertices=0;
    vis_vertices=0;
    idxn=0;
    for(int i=0;i<7;i++){
...
// Phase N Stage 1: opt-in mesh fingerprinting. Default OFF — this is a pass over every published
// vertex, and prepareVBO() runs for every re-meshed chunk. eden_debug_set_mesh_checksum() (a
// diagnostics-only export, web/src/seam/DebugState_web.mm) is the only writer.
bool g_mesh_checksum_enabled = false;

// FNV-1a 64. Not a security hash — chosen because it is six lines with no dependency and its
// avalanche is far more than enough to catch a shifted vertex. `pad` is deliberately NOT hashed:
// it is the 2-byte alignment filler in vertexStructSmall (Graphics.h) that the mesher never
// assigns, so including it would make the fingerprint a function of allocation history.
static inline void eden_mesh_hash_span(unsigned long long& h, const void* p, unsigned n){
    const unsigned char* b=(const unsigned char*)p;
    for(unsigned i=0;i<n;i++){ h^=b[i]; h*=1099511628211ULL; }
}
static void eden_mesh_hash_vertices(unsigned long long& geom, unsigned long long& full,
                                    const vertexStructSmall* v, int count){
    eden_mesh_hash_span(geom,&count,sizeof(count));
    eden_mesh_hash_span(full,&count,sizeof(count));
    if(!v||count<=0) return;
    for(int i=0;i<count;i++){
        eden_mesh_hash_span(geom,v[i].position,sizeof(v[i].position));
        eden_mesh_hash_span(geom,v[i].texs,sizeof(v[i].texs));
        eden_mesh_hash_span(full,v[i].position,sizeof(v[i].position));
        eden_mesh_hash_span(full,v[i].texs,sizeof(v[i].texs));
        eden_mesh_hash_span(full,v[i].colors,sizeof(v[i].colors));
    }
}

void TerrainChunk::prepareVBO(){
    rebuildCounter=0;
    // Before anything below frees or zeroes them. n_vertices/n_vertices2 (not the rt* copies) are
    // the counts that match verticesbg/verticesbg2 at this instant — the rt* ones are only
    // assigned from them further down.
    if(g_mesh_checksum_enabled){
        if(clearOldVerticesOnly||!needsVBO){
            if(clearOldVerticesOnly){ rtGeomHash=0; rtFullHash=0; }
        } else {
            unsigned long long geom=1469598103934665603ULL, full=1469598103934665603ULL;
            eden_mesh_hash_vertices(geom,full,verticesbg,n_vertices);
            eden_mesh_hash_vertices(geom,full,verticesbg2,n_vertices2);
            rtGeomHash=geom; rtFullHash=full;
        }
    }
...
void TerrainChunk::render2(){
    ...
    BOOL v5=cam->py>=pbounds[1]&&rtnum_vertices2[5]!=0;
    BOOL v1=cam->pz>=pbounds[2]&&rtnum_vertices2[1]!=0;
    BOOL v0=cam->pz<=pbounds[5]&&rtnum_vertices2[0]!=0;
    BOOL v3=cam->px>=pbounds[0]&&rtnum_vertices2[3]!=0;
    BOOL v2=cam->px<=pbounds[3]&&rtnum_vertices2[2]!=0;
    BOOL v4=cam->py<=pbounds[4]&&rtnum_vertices2[4]!=0;

    // Row #20 (perf-audit): faces 0-5 are laid out back-to-back in vertexBuffer2 by construction
    // (face_idx2[i]=num_vertices2[i-1]+face_idx2[i-1] above), so when EVERY face is being drawn —
    // the common case, since it just means the camera sits within this chunk's XYZ span — the six
    // draws below collapse into one glDrawArrays over the whole contiguous span instead of six
    // separate ones. This does submit the six faces in ascending-index order (0,1,2,3,4,5) rather
    // than the scrambled per-axis order the individual calls use, which only matters for
    // translucent overdraw between two DIFFERENT faces of the SAME chunk landing on the same
    // screen pixel at overlapping depth — for this stream (water/lava/glass/leaves quads facing
    // six different directions of one 16x16x64 chunk) that is not a real-world case, and the
    // cross-CHUNK back-to-front order this pass depends on (Terrain::render2's renderList2 qsort,
    // docs/rendering.md) is untouched either way. The partial-visibility case (fewer than six
    // faces — typically a chunk at the world's edge, where an order change would be most visible)
    // is deliberately left on the original per-face path below, unmodified.
    if(v0&&v1&&v2&&v3&&v4&&v5){
        int total=rtnum_vertices2[0]+rtnum_vertices2[1]+rtnum_vertices2[2]+
                   rtnum_vertices2[3]+rtnum_vertices2[4]+rtnum_vertices2[5];
        glDrawArrays(GL_TRIANGLES, rtface_idx2[0], total);
    }else{
        if(v5) glDrawArrays(GL_TRIANGLES, rtface_idx2[5], rtnum_vertices2[5]);
        if(v1) glDrawArrays(GL_TRIANGLES, rtface_idx2[1], rtnum_vertices2[1]);
        if(v0) glDrawArrays(GL_TRIANGLES, rtface_idx2[0], rtnum_vertices2[0]);
        if(v3) glDrawArrays(GL_TRIANGLES, rtface_idx2[3], rtnum_vertices2[3]);
        if(v2) glDrawArrays(GL_TRIANGLES, rtface_idx2[2], rtnum_vertices2[2]);
        if(v4) glDrawArrays(GL_TRIANGLES, rtface_idx2[4], rtnum_vertices2[4]);
    }
